import { CartItem } from '@/libs/Types/Cart/Index';
...
const Cart: React.FC = () => {
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([
    {
      slug: 1,
      name: 'Kith for the New York Rangers Skewed Delk Hoodie - Nocturnal1',
      price: 21100,
      size: 'M',
    ...
    {
      slug: 2,
      name: "Kith & '47 for the New York Rangers Franchise LS Cap - Nocturnal",
      price: 5800,
      size: 'L',
      image: "/ProductCard-Images/MENS_WINTER_LOOKBOOK_LOOK7_SHOT_1_0945_720x.webp",
      quantity: 1,
  ...
  const handleQuantityChange = (slug: number, delta: number) => {
  ...
  return (
    <>
     <button className="fixed right-[0px] p-3 rounded-full z-50 relative" onClick={handleToggleCart}>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[400px] bg-white shadow-xl transform transition-transform duration-300 z-50 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* Cart Header - Fixed */}
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-black text-lg font-['sens-serf']">Shopping Cart</h2>
          <button className="text-black transform uppercase font-bold text-[11px]" onClick={handleToggleCart}>
            Close
          </button>
        </div>

        {/* Scrollable Container for Cart Items and Recommendations */}
        <div className="flex-1 overflow-y-auto">
          {/* Cart Items */}
          <div className="p-6">
            {cartItems.map((item) => (
              <div key={item.slug} className="flex gap-4 mb-4 pb-4 border-b border-gray-200 last:border-b-0">
